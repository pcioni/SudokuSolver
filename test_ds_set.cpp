...
int main() {
  ...
  ds_set<int>::iterator d;
  d = set5.find(5);
  if (d == set5.end())
    std::cout << "\"5\" is not in the set\n";
  else
    std::cout << "\"5\" is in the set\n"
        << "The iterator points to " << *d << std::endl;
  ...
  std::cout << "set 3 size:" << set3.size() << std::endl;
  ds_set<std::string> set4(set3);
  std::cout << "set 4 size:" << set4.size() << std::endl;
  set4.insert(std::string("happy halloween!"));
  std::cout<<"set3 tree:"<<std::endl;
  set3.print_as_sideways_tree(std::cout);
  std::cout<<"set4 tree:"<<std::endl;
